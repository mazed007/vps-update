...
<div class="row">
    ...
    <div class="col-sm-12 col-md-8 col-lg-8">
        <h3 class="page-title">Edit Pin</h3>
        <form class="edit-form" method="post" action="{{ route('pin.update', $pin->id) }}">
        	{{ csrf_field() }}
        	{{ method_field('put') }}
            <label>Email</label>
            ...
            <select name="period" id="period" class="period">
                @foreach(vps_periods() as $value => $period)
                    <option value="{{ $value }}" @if($value == $pin->period) selected @endif>{{ $period }}</option>
                @endforeach
            </select>
